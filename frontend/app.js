...
async function loadCandidates() {
    ...
    const skip = Math.max(
        0,
        parseInt(document.getElementById('candidate-skip').value) || 0
    );

    const limit = Math.max(
        1,
        parseInt(document.getElementById('candidate-limit').value) || 50
    );
...
function clearCandidateFilters() {
    ...
    document.getElementById('candidate-skip').value = '0';
    document.getElementById('candidate-limit').value = '50';
...
function editCandidate(c) {
    ...
    // Scroll to form
    document.getElementById('candidate-form').scrollIntoView({
        behavior: 'smooth',
        block: 'center'
    });
}
...
async function loadStatuses() {
    ...
    const candidateIdDirect = document.getElementById('filter-candidate-id').value;
    ...
    const skip = Math.max(
        0,
        parseInt(document.getElementById('status-skip').value) || 0
    );

    const limit = Math.max(
        1,
        parseInt(document.getElementById('status-limit').value) || 50
    );
    ...
    // Candidate ID input
    if (candidateIdDirect) {
        params.append('candidate_id', candidateIdDirect);
    }

    // Exact date
    if (exactDate) {
        params.append('status_date', exactDate);
    }
                ...
                <tr>
                    ...
                    <td>
                        <div class="pct-bar-wrap">
                            <div class="pct-bar">
                                <div class="pct-fill ${pctClass}" style="width:${pct}%"></div>
                            </div>
                            <span>${pct}%</span>
                        </div>
                    </td>
...
async function handleStatusSubmit(e) {
    ...
    if (!body.work_completed) {
        showAlert('Work Completed is required.', true);
        return;
    }

    if (!body.topics_learned) {
        showAlert('Topics Learned is required.', true);
        return;
    }

    if (!body.next_day_plan) {
        showAlert('Next Day Plan is required.', true);
        return;
    }

    if (
        isNaN(body.completion_percentage) ||
...
function clearStatusFilters() {
    ...
    document.getElementById('status-skip').value = '0';
    document.getElementById('status-limit').value = '50';
